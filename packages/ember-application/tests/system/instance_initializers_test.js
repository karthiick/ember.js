...
moduleFor('Ember.Application instance initializers', class extends AutobootApplicationTestCase {
  ...
  [`@test initializers require proper 'name' and 'initialize' properties`]() {
    expectAssertion(() => {
      run(() => {
        this.applicationInstance.instanceInitializer({ name: 'initializer' });
      });
    });

    expectAssertion(() => {
      run(() => {
        this.applicationInstance.instanceInitializer({ initialize() {} });
      });
    });
  },

  [`@test initializers are passed an app instance`](assert) {
    this.applicationInstance.instanceInitializer({
      name: 'initializer',
      initialize(instance) {
